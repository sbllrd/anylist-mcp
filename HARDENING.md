...
- ~~**Open**: `cloudflared-named` still uses `cloudflare/cloudflared:latest` — a floating tag on the container that holds the tunnel credential (pin-dependencies principle). Pin to a digest.~~ **Pinned 2026-09-24**: both `cloudflared-named` and `cloudflared-temp` use `cloudflare/cloudflared:2026.9.3@sha256:072c067d…` — the exact image already running (tag and running container confirmed to resolve to the same digest). To update: `docker buildx imagetools inspect cloudflare/cloudflared:<new-version>`, replace both version+digest in `docker-compose.yml`, recreate `cloudflared-named`, and check the tunnel registers.
